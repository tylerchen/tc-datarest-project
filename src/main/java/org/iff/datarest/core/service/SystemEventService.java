...
import static org.iff.datarest.util.SystemHelper.NAMESPACE_SYS;
import static org.iff.datarest.util.SystemHelper.key;
...
public class SystemEventService implements EventBusHelper.EventProcess {
    ...
    public DomainQueryModel getDomainQueryModel(DomainQueryModel model) {
        if (NAMESPACE_SYS.equals(model.getNamespace())) {
            String queryDsl = CacheHelper.getCache().get(key(model.getModelName(), model.getQueryName(), model.getNamespace()));
            if (StringUtils.isNotBlank(queryDsl)) {
                model.setQueryContent(queryDsl);
                return model;
            }
        }
    ...
    public static class Tuple<T> extends org.iff.datarest.util.Tuple.Six<T, String /*action*/, DomainQueryModel /*dqm*/, DomainParamModel /*dpm*/, DataSourceModel /*dsm*/, Object /*id*/, Page /*page*/> {

        public Tuple(String action, DomainQueryModel dqm, DomainParamModel dpm, DataSourceModel dsm, Object id, Page page) {
            super(action, dqm, dpm, dsm, id, page);
        }
