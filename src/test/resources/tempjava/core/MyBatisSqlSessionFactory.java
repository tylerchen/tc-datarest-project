...
import org.iff.datarest.core.mybatis.MybatisSqlServiceImpl;
...
public class MyBatisSqlSessionFactory {
    ...
    public static RepositoryService service(String dataSourceName) {
        org.iff.datarest.core.mybatis.MybatisSqlServiceImpl service = new org.iff.datarest.core.mybatis.MybatisSqlServiceImpl();
        service.setSqlSession(create(dataSourceName).openSession());
        return service;
    }

    public static void close(RepositoryService service) {
        ((org.iff.datarest.core.mybatis.MybatisSqlServiceImpl) service).close();
    }


    public static void commit(RepositoryService service) {
        ((org.iff.datarest.core.mybatis.MybatisSqlServiceImpl) service).commit();
    }
    ...
    public static void done(boolean isRollback) {
        ...
        if (isRollback) {
            ...
            if (MapUtils.isNotEmpty(map)) {
                for (Map.Entry<String, RepositoryService> entry : map.entrySet()) {
                    org.iff.datarest.core.mybatis.MyBatisSqlSessionFactory.commit(entry.getValue());
                }
            }
        }
