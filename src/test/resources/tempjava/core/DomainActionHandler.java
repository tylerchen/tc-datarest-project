    ...
    public boolean execute(ProcessContext ctx) {
        ...
        if (restUri.length > 1 && "ex".equals(restUri[0])) {
            ...
            EventBusHelper.me().syncEvent(
                    org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                    org.iff.datarest.core.service.DomainEventService.Tuple.extra(namespace, modelName, name, pathConditions, ctx));
        } else if (ctx.isGet()) {
            if (restUri.length > 1 && "find".equals(restUri[0])) {
                ...
                EventBusHelper.me().syncEvent(
                        org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                        org.iff.datarest.core.service.DomainEventService.Tuple.find(namespace, modelName, pathConditions, ctx));
            } else if (restUri.length == 1) {
                // GET    /:id                   -> articles#show
                EventBusHelper.me().syncEvent(
                        org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                        org.iff.datarest.core.service.DomainEventService.Tuple.show(namespace, modelName, restUri[1], ctx));
            } else if (restUri.length == 0) {
                // GET    /                      -> articles#index
                EventBusHelper.me().syncEvent(
                        org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                        org.iff.datarest.core.service.DomainEventService.Tuple.index(namespace, modelName, ctx));
            } else {
            ...
            if (restUri.length == 0) {
                ...
                EventBusHelper.me().syncEvent(
                        org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                        org.iff.datarest.core.service.DomainEventService.Tuple.create(namespace, modelName, ctx));
            } else {
            ...
            if (restUri.length == 1) {
                ...
                EventBusHelper.me().syncEvent(
                        org.iff.datarest.core.service.DomainEventService.EVENT_NAME,
                        org.iff.datarest.core.service.DomainEventService.Tuple.update(namespace, modelName, restUri[1], ctx));
            } else {
        ...
        if (hasError()) {
            if (MapUtils.isNotEmpty(map)) {
                for (Map.Entry<String, RepositoryService> entry : map.entrySet()) {
                    org.iff.datarest.core.mybatis.MyBatisSqlSessionFactory.rollback(entry.getValue());
                }
            }
        } else {
            if (MapUtils.isNotEmpty(map)) {
